...
contract FastCoin is ERC20 {
    ...
    function transfer(
        ...
        require(
            ...
            "You dont have enough balance to transfer the amount"
        );
        balances[msg.sender] -= amount;
    ...
    function approve(address spender, uint256 value) external returns (bool) {
        require(
            ...
            "You dont have enough balance to set the allowance amount"
        );
    ...
    function transferFrom(
        ...
        require(
            ...
            "The sender balance is less than the amount"
        );
        require(
            allowances[sender][msg.sender] >= amount,
            "The allowance for contract invoker is less than the amount"
        );
...
contract MenuManagement {
    ...
    function updatePrice(
        uint256 _index,
        uint256 _itemPrice
    ) public onlyOwner indexCheck(_index) {
    ...
    function getPrice(
        uint256 _index
    ) public view indexCheck(_index) returns (uint256) {
    ...
    function checkAvailability(
        uint256 _index
    ) public view indexCheck(_index) returns (bool) {
    ...
    function getAllItemNames() public view returns (string[] memory) {
        string[] memory itemNames = new string[](menuItems.length);
        for (uint256 i = 0; i < menuItems.length; i++) {
            itemNames[i] = menuItems[i].itemName;
        }
        return itemNames;
    }

    function getAllItemPrices() public view returns (uint256[] memory) {
        uint256[] memory itemPrices = new uint256[](menuItems.length);
        for (uint256 i = 0; i < menuItems.length; i++) {
            itemPrices[i] = menuItems[i].price;
        }
        return itemPrices;
    }

    function getAllItemQuantities() public view returns (uint256[] memory) {
        uint256[] memory itemQuantities = new uint256[](menuItems.length);
        for (uint256 i = 0; i < menuItems.length; i++) {
            itemQuantities[i] = menuItems[i].quantity;
        }
...
contract OrderContract {
    ...
    function calculateOrderAmount(
        uint256[] memory _itemIndexes,
        uint256[] memory _quantities
    ) public view returns (uint256) {
        require(
            _itemIndexes.length == _quantities.length,
            "Error: Item and quantity count mismatch"
        );
    ...
    function placeOrder(
        ...
        for (uint256 i = 0; i < _itemIndexes.length; i++) {
            mm.updateQuantity(
                _itemIndexes[i],
                mm.getItemQuantity(_itemIndexes[i]) - _quantities[i]
            );
        }
